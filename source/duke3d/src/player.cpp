...
void P_UpdateAngles(int const playerNum, input_t const &input)
{
    ...
    if (pPlayer->aim_mode&2 && !input.q16avel && !input.q16horz && input.fvel)
    {
...
// FIXME
    if (pPlayer->aim_mode&2 && ud.config.JoystickViewLeveling
        && GetAutoAimAng(pPlayer->i, playerNum, 0, ZOFFSET3, 0 + 2, &pPlayer->pos, 256, &Zvel, &shootAng) != -1)
    {
